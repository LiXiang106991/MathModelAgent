...
本 skill 承接 `2analysis-modeling`。目标是把 `reports/ANALYSIS_MODELING_REPORT.md` 里的模型和算法落实为可复现程序，跑出可信结果，并通过SPSS软件或其他方式生成论文中需要的数据型图表。
...
本电脑已安装用于统计分析、实现结果可视化的SPSS软件，如果问题属于回归分析，预测建模等，可调用SPSS API，在这个软件上操控，实现数据可视化，构造图表
