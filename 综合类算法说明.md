...
def plot_reaction_functions(payoff_matrix_a, payoff_matrix_b):
    """
    绘制反应函数图（适用于2×2博弈）
    :param payoff_matrix_a: 玩家A的收益矩阵 shape (2,2)，行=A的纯策略，列=B的纯策略
    :param payoff_matrix_b: 玩家B的收益矩阵 shape (2,2)
    """
    # 计算玩家A的最优响应 BR_A(q)：给定B选择策略1的概率 q，A选策略1/2的期望收益
    q = np.linspace(0, 1, 200)
    # A选策略1的期望收益 = q*A11 + (1-q)*A12；选策略2 = q*A21 + (1-q)*A22
    E_A1 = q * payoff_matrix_a[0, 0] + (1 - q) * payoff_matrix_a[0, 1]
    E_A2 = q * payoff_matrix_a[1, 0] + (1 - q) * payoff_matrix_a[1, 1]
    # A的最优响应：E_A1 >= E_A2 时选策略1（概率1），否则选策略2（概率0）
    BR_A = (E_A1 >= E_A2).astype(float)

    # 玩家B的最优响应 BR_B(p)：给定A选策略1的概率 p
    p = np.linspace(0, 1, 200)
    E_B1 = p * payoff_matrix_b[0, 0] + (1 - p) * payoff_matrix_b[1, 0]  # B选策略1
    E_B2 = p * payoff_matrix_b[0, 1] + (1 - p) * payoff_matrix_b[1, 1]  # B选策略2
    BR_B = (E_B1 >= E_B2).astype(float)

    plt.figure(figsize=(8, 8))
    plt.plot(q, BR_A, 'r-', linewidth=2, label='Player A Best Response')
    plt.plot(BR_B, p, 'b-', linewidth=2, label='Player B Best Response')
    ...
    # 返回纯策略纳什均衡（反应函数交点，即双方同时最优响应的策略组合）
    nash = []
    for i in range(2):
        for j in range(2):
            # 检查 (i,j) 是否双方最优响应
            if (payoff_matrix_a[i, j] >= payoff_matrix_a[1 - i, j] and
                    payoff_matrix_b[i, j] >= payoff_matrix_b[i, 1 - j]):
                nash.append((i, j))
    return nash
```
